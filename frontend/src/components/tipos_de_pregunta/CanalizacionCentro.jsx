...
import { Box, Select, MenuItem, Typography, FormControl, Button, useTheme, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle } from "@mui/material";
...
const candidateIcon = 'http://maps.google.com/mapfiles/ms/icons/homegardenbusiness.png'; // Example icon
const centerIcon = 'http://maps.google.com/mapfiles/ms/icons/green-dot.png'; // Example icon
const selectedCenterIcon = 'http://maps.google.com/mapfiles/ms/icons/blue-dot.png'; // Example icon
const currentCenterIcon = 'http://maps.google.com/mapfiles/ms/icons/yellow-dot.png'; // Example icon
...
const CanalizacionCentro = ({
    ...
    useEffect(() => {
        if (candidate) {
            ...
            // Make sure to update seleccionOpcion with the correct center_id
            if (!seleccionOpcion?.center_id || seleccionOpcion.center_id !== centerId) {
                setSeleccionOpcion({ center_id: centerId });
            }
        }
    ...
    const handleFinalConfirmChange = async () => {
        if (usuarioId && newCenterId) {
            try {
                ...
                setError(error.response?.data?.detail || "Error al solicitar el cambio de centro.");
            }
        } else {
            setShowFinalConfirmationDialog(false);
            setIsConfirmationOpen(false);
    ...
    const fetchCandidateAndCenters = useCallback(async () => {
        try {
            ...
            const candidateRes = await axios.get(`/api/candidatos/${usuarioId}/canalizar-centro/`);
    ...
    const calculateDistance = useCallback((lat1, lng1, lat2, lng2) => {
        if (isNaN(lat1) || isNaN(lng1) || isNaN(lat2) || isNaN(lng2)) return 'Calculando...';
    ...
    const currentSelection = newCenterId || displayCenterId || candidate?.center?.id || "";
    ...
    return (
        ...
        >
            ...
            <FormControl fullWidth>
                <Select
                    ...
                    renderValue={(selected) => {
                        ...
                        const selectedCenter = centers.find((center) => center.id === selected);
                        return selectedCenter ? `${selectedCenter.name} (${distances[selectedCenter.id] || 'Calculando...'})` : "Centro no encontrado";
                    }}
                    sx={{
                        fontSize: "1.1rem",
                        borderRadius: 1,
                        boxShadow: 1,
                        backgroundColor: candidate?.center?.id === currentSelection && colors.lightGreen[600],
                        color: candidate?.center?.id === currentSelection && "black",
                    }}
                >
                    <MenuItem disabled value="">Seleccionar centro</MenuItem>
                    {centers
            ...
            {isLoaded && candidate !== null && (
                ...
                >
                    {candidateLocation && !isNaN(candidateLocation.lat) && !isNaN(candidateLocation.lng) && (
                        <Marker position={candidateLocation} title="Ubicación del Candidato" icon={candidateIcon} />
                    )}
                    {centers.map(center => {
                        ...
                        if (!isNaN(lat) && !isNaN(lng)) {
                            ...
                            return (
                                ...
                                >
                                    {isClicked && (
                                        <InfoWindow onCloseClick={() => setSelectedCenterForMap(null)}>
                                            <Box>
                                                <Typography variant="subtitle1" sx={{ color: "black"}} >{selectedCenterForMap.name}</Typography>
                                                <Typography sx={{ color: "black"}} >Distancia: {distances[selectedCenterForMap.id] || 'Calculando...'}</Typography>
                                            </Box>
                                        </InfoWindow>
                                    )}
                                </Marker>
            ...
            <Box sx={{ mt: 2, display: 'flex', gap: 2, alignItems: 'center', overflowX: 'auto' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Typography variant="body2">Centro Seleccionado (Dropdown)</Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {isConfirmationOpen && (
                <Box sx={{ mt: 2, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                    <Button color='secondary' onClick={handleCancelChange}>Cancelar</Button>
                    <Button variant="contained" color="primary" onClick={handleInitialConfirmChange}>
                        Confirmar Cambio
                    </Button>
                </Box>
            )}
            <Box display='flex' sx={{ mt: 2}}>
                ...
                <Typography variant="h6" color="warning" sx={{ ml: 1 }}>
                    Recuerda: al cambiar el centro del candidato a uno distinto al tuyo, ya no podrás visualizar o editar al mismo.
                </Typography>
            </Box>
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        Al confirmar, se enviará una solicitud de transferencia. Una vez aceptada, el candidato será asignado a un nuevo centro y ya no podrás visualizar ni editar su información. ¿Deseas continuar?
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleCloseFinalConfirmationDialog} color="secondary">
                        Cancelar
                    </Button>
                    <Button onClick={handleFinalConfirmChange} color="primary" autoFocus variant="contained">
                        Confirmar Solicitud
                    </Button>
                </DialogActions>
            </Dialog>
