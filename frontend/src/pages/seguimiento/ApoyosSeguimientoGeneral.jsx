...
import CuadroHabilidadesAidHistory from "./tipos/CuadroHabilidadesAidHistory";
import {
  Accordion,
  AccordionSummary,
  AccordionDetails,
    ...
    return (
      <Paper
        ...
        sx={{
          p: 2,
          my: 1.5,
          borderLeft: isActiva ? "6px solid #1976d2" : "6px solid #ccc",
          backgroundColor: isActiva ? "#e3f2fd" : "#fafafa",
          ...
          <Grid item xs={12} sm={3} textAlign={{ xs: "left", sm: "right" }}>
            {estado?.activo ? (
              estado.resultado === "funciono" ? (
                ...
                <HighlightOffIcon sx={{ fontSize: 28, color: "red" }} />
              ) : (
                <HourglassEmptyIcon sx={{ fontSize: 28, color: "orange" }} />
              )
            ) : (
...
const ApoyosSeguimientoGeneral = ({ uid, tipo = "sis" }) => {
  ...
  const [resumenRenderizado, setResumenRenderizado] = useState(null);

  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const endpoints = endpointMap[tipo] || endpointMap["sis"];
      const res = await axios.get(
        `/api/cuestionarios/${endpoints.resumen}/?usuario_id=${uid}`
      );

      if (tipo === "ch") {
        console.log("🧠 Datos resumen CH:", res.data);
        setResumenRenderizado(res.data); // guarda el objeto resumen completo
        return;
      }
      ...
      setResumenRenderizado(null);
    } catch (err) {
      console.error(`Error al cargar resumen ${tipo.toUpperCase()}:`, err);
  ...
  useEffect(() => {
    ...
    const endpoints = endpointMap[tipo] || endpointMap["sis"];
    axios
      .get(`/api/candidatos/seguimiento/${endpoints.seguimiento}/${uid}/`)
      .then((res) => {
  ...
  const fetchHistorial = async (ayudaId) => {
    try {
      const endpoints = endpointMap[tipo] || endpointMap["sis"];
      const res = await axios.get(
        `/api/candidatos/seguimiento/${endpoints.seguimiento}/${uid}/history/`
      );
  ...
  const handleSetActiva = useCallback(
    (subitemKey, ayudaId) => {
      ...
      setTimeout(() => {
        ...
        axios
          .post(
            `/api/candidatos/seguimiento/${endpoints.seguimiento}/`,
            payload
          )
          .then(() => {
    ...
    [estadoPorAyuda, secciones, uid, tipo]
  );
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      {tipo === "ch" && <CuadroHabilidadesAidHistory resumen={resumenRenderizado} />}
...
export default React.memo(ApoyosSeguimientoGeneral);
