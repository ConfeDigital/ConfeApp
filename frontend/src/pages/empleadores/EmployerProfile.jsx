...
export default function Profile() {
  ...
  const handleGetEmployerProfile = async () => {
    try {
      const res = await axios.get("/api/agencia/employer/me/");
  ...
  return (
    <Box flex={1} p={4}>
      ...
      >
        <Avatar
          {...stringAvatar(`${user.first_name} ${user.last_name}`, 80, "3rem")}
        />
        <Box flex={1}>
      ...
      <Box p={3} boxShadow={2} borderRadius={2} mb={2}>
        ...
        <Divider sx={{ mb: 2 }} />
        <Typography variant="h5">{currentEmployer.company_name}</Typography>
      </Box>
